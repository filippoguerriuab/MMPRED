...
class CNNPepPred:
    def __init__(self,allele='no_allele_name',savePath=Path(os.getcwd()),doTraining=False,trainingData=None,trainingOutcome=None,doLogoSeq=False,doCV=False,cvPart=None,kFold=5,doApplyData=False,trainedModelsFile=None,applyData=None,applyDataName=None,epitopesLength=15,parametersFile='parameters.txt'):
        ...
        os.mkdir(self.savePath, mode=999)
    ...
    def getParameters(self):
        ...
        if similarityMat!=similarityMat:
            similarityMat = './bin/blosum62.txt'
    ...
    def getCoreBinder(self,seqs,contributionScore,applyDataName=None,saveCoreBinders=False):

        """
        print('+++++++++++++')
        print(seqs)
        print(len(seqs[0]))
        print(contributionScore.shape)
        """
        # filippo's new code
        ...
        for si in range(len(seqs)):
            seq = seqs[si]
            currContributionScore = np.array([contributionScore[i,si] for i in range(len(contributionScore[:,si])) if seq[i] != '-'])
            seq = ''.join([seq[i] for i in range(len(seq)) if seq[i]!= '-'])
            indMax = np.argmax(currContributionScore[:len(currContributionScore)-l])
            curr_sCore = seq[indMax:indMax+l]
            sCore.append(curr_sCore)


        if saveCoreBinders:
            self.coreBinders = np.array(sCore)
        return sCore
        
        """
        # valentine's old code, BUG: sCOre is returning with gaps
        l = self.l
        indMax = np.argmax(contributionScore,0)
        sCore = np.array([seqs[i][indMax[i]:indMax[i]+l] for i in np.arange(len(seqs))])
        if saveCoreBinders:
